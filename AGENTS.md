...
`독서토론앱.md`의 "Codex / Gemini CLI 인계 프롬프트" 절은 이 저장소를 리뷰 전용으로 여는 세션을 위한 별도 지시사항이다 — 코드를 수정하지 말고 `docs/SECURITY.md`의 "외부 리뷰 점검 항목"에 따라 보고서만 작성하는 작업이 요청되면 그 절차를 따를 것.
