...
- `middleware.ts`가 `/login`과 `/api/auth/*`를 제외한 전 경로를 보호한다.
...
| reply(사유 더하기) 작성 | 불가 | 가능(`topic.kind='excerpt'`이고 `session.status='open'`일 때만. 본인 answer에 본인이 다는 것도 허용) | 가능 |
...
answers/replies를 쓰는 모든 Server Action은 소유권 검사에 더해 아래 두 가지도 함께 서버에서 확인한다(DECISIONS.md의 Phase 0 후속 결정 참고):

- **회차 상태 검사**: 대상 `answer`/`reply`가 속한 `session.status === 'open'`인지 확인. `draft`나 `closed`면 거부한다. 상태별로 분기하지 않고 이 한 조건으로 draft/closed 둘 다 막는다.
- **reply 생성 시 topic kind 검사**: 대상 `answer`가 속한 `topic.kind === 'excerpt'`인지 확인. `free`(또는 `choice`)면 거부한다. 이 검사는 DB 트리거가 아니라 Server Action에서 한다.
...
- [ ] `topic.kind !== 'excerpt'`인 answer에 reply 생성을 시도하면 서버가 거부하는가.
...
- [ ] 빌드 산출물(`.next`)에 `SUPABASE_SERVICE_ROLE_KEY` 문자열이 없는가 — grep으로 실제 확인.
- [ ] 클라이언트 컴포넌트(`"use client"`)에서 `@supabase/supabase-js`를 import하는 곳이 없는가.
- [ ] 전 테이블에 RLS가 켜져 있는가(정책 0개인 것도 함께 확인 — 정책이 실수로 추가돼 있으면 오히려 의도치 않게 열릴 수 있다).
- [ ] `middleware.ts`의 matcher에서 빠진 경로가 없는가(새 라우트 추가 시 matcher 갱신을 잊기 쉬움).
