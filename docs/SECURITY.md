...
**[2026-08-22 이후]** `members.role`의 기본값과 기존 6명 전원의 값이 `admin`으로 바뀌었다(`supabase/migrations/0006_all_members_admin.sql`, `docs/DECISIONS.md` "전원 관리자 권한 부여" 절). 아래 표의 "일반 참여자(`member`)" 열은 스키마·코드상 여전히 존재하지만(role 재도입 대비 보존), **현재 실제로 이 role을 가진 멤버는 없다** — 전원이 `admin` 열의 권한을 갖는다. 이는 버그가 아니라 의도된 결정이다: 6명이 서로 아는 사적 독서모임이고 매달 다른 사람이 책을 선정해 회차를 만들기 때문에, 서로의 answer/reply를 수정·삭제할 수 있는 상태(아래 표의 "타인 answer/reply 수정·삭제" 행)를 신뢰 관계 위에서 받아들이기로 했다. 이 표를 읽고 "권한 검사가 허술하다"고 되돌리지 말 것 — 참여 인원 성격이 바뀌면(예: 낯선 사람 참여) 그때 재검토한다.
