...
### 전원 관리자 권한 부여 + `/admin` 무권한 접근 안내 개선 (2026-08-22)

**결정**: `members.role`을 전원 `admin`으로 통일하고(마이그레이션 `0006_all_members_admin.sql`, 미실행 — 사람이 직접 적용), 이후 추가되는 멤버의 기본값도 `admin`으로 바꾼다. `role` 컬럼과 체크 제약(`role in ('member','admin')`) 자체는 남겨둔다 — 값만 통일하고 스키마는 훗날 역할을 재도입할 여지를 보존한다. 동시에 `requireAdmin()`(`lib/auth.ts`)과 `proxy.ts`가 무권한 `/admin` 접근을 `"/"`로 안내 없이 리다이렉트하던 것을 신설한 `/forbidden` 페이지("이 화면은 관리자만 볼 수 있습니다" + 홈 링크)로 바꿨다.

**이유**: 2026-08-22 실제 모임에서 admin이 수열 한 명뿐이고 나머지 5명이 `member`였던 상태로 `/admin`에 접근했을 때, 미들웨어와 `requireAdmin()` 둘 다 조용히 `/`로 돌려보내는 바람에 "관리자 화면에서 다른 멤버가 안 보인다"는 버그 리포트로 이어졌다(코드·DB 양쪽을 조사한 결과, 실제로는 멤버 목록 자체는 정상이었고 — 6명 전원 `is_active=true` — 애초에 관리자가 아니라서 화면 자체를 못 본 것이었다). 이 앱은 6명이 서로 아는 사적 독서모임이고 매달 다른 사람이 책을 선정해 회차를 만드는 구조라, `admin`/`member`를 나눠 관리할 실익이 없다고 판단해 권한 자체를 통일하기로 했다. 다만 "권한이 없어 화면이 안 보인다"는 상황 자체는 role을 어떻게 나누든 재발할 수 있으므로(신규 멤버가 아직 admin으로 승격되기 전 등), 안내 없는 리다이렉트라는 근본 원인도 함께 고쳤다.

**주의**: 전원 `admin`이 되면 서로의 answer/reply를 수정·삭제할 수 있다 — 이는 실수가 아니라 이 결정의 직접적 결과이며 신뢰 관계 위에서 받아들인 것이다(`docs/SECURITY.md` "권한 매트릭스" 상단 주석 참고). 나중에 이 코드를 읽고 "권한 검사가 허술하다"고 되돌리지 말 것 — 참여 인원 성격이 바뀌면(예: 낯선 사람 참여) 그때 다시 role을 나누는 것을 검토한다.
