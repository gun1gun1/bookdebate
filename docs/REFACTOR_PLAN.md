...
| 단계 | 내용 | DB 변경 |
|---|---|---|
| **R1-b** | 레이아웃 개편(사이드바+스트림) + 브랜딩("내담리", 히어로, 헤더 로고, 파비콘) | 없음 |
| **R1-a** | 스키마 마이그레이션(`MIGRATION_R1.md`) | 있음 |
| **R1-c** | 논제 5종 UI(`DifficultView`/`ChoiceView`/`AppendixView` 신설, `ExcerptView` 회색카드화) + 모든 kind에 "의견 남기기" + 이관 파서 확장 | 없음(R1-a가 이미 적용된 스키마를 사용) |
| **R1-d** | 정리 및 점검(구 코드/문서 잔재 제거, `SECURITY.md` 체크리스트 재확인, `.next` grep 등 배포 전 점검) | 없음 |
...
- `docs/SCHEMA.md`: `docs/SCHEMA_R1_DRAFT.md`(갱신됨) 내용으로 교체.
...
- `topics.choice_options`(신규 컬럼, `MIGRATION_R1.md` 참고)를 선택지 원본으로 쓴다 — 관리자가 "찬성/반대" 대신 "A안/B안" 등으로 바꿔도 이 화면은 그대로 동작.
...
1. **`answers_topic_id_member_id_key` → `answers_topic_member_slot_key` 전환 (위험도: 낮음, v1의 "중간"에서 하향)**
   ...
   - 남는 위험은 `appendix`에 한정된다: 롤백 시(구 2컬럼 제약으로 되돌릴 때) `appendix` 논제에 실제로 `slot > 0`인 다건 게시물이 쌓여 있다면, 그중 하나만 남기고 나머지를 버리는 판단이 필요하다 — 이 판단 자체는 여전히 사람이 해야 한다(MIGRATION_R1.md 롤백 절 참고).
